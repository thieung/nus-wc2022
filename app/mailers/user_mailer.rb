class UserMailer < ApplicationMailer
  ...
  def send_password_to_staff user, password
    ...
    @password = password
    @url = login_url
    mail(to: @user.email, subject: "[NUS Event - World Cup 2022] Thông tin tài khoản")
  end
  ...
  def notice_bet_result_to_staffs recipient, bet_info
    ...
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Dự đoán tỉ số trận #{@match_content}")
  end
  ...
  def notice_bet_result_updated_to_staffs recipient, bet_info
    ...
    @game = bet_info.game
    @match_content = "#{@game.team1.title_vi} - #{@game.team2.title_vi}"
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] [Update] Dự đoán tỉ số trận #{@match_content}")
  end
  ...
  def notice_match_result_to_staffs recipient, game
    ...
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Kết quả trận #{@match_content}")
  end
  ...
  def notice_predict_champion_result_to_staffs recipient, predict_champion
    ...
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Dự đoán đội vô địch lần #{@user.total_teams_to_predict_champion}")
  end

  def notice_all_bets_info_to_staffs recipient, game
    @game = game
    @match_content = "#{@game.team1.title_vi} - #{@game.team2.title_vi}"
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Danh sách nhà đầu tư dự đoán tỉ số trận #{@match_content}")
  end
  ...
  def notice_champion_winner_result_to_staffs recipient
    ...
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Kết quả dự đoán đội vô địch World Cup 2022")
  end

  def staffs_top_scores_report_after_first_round_group_stage recipient
    @statistics = User.top_scores_report
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Tổng hợp kết quả sau lượt trận đầu tiên vòng đấu bảng")
  end

  def staffs_top_scores_report_after_second_round_group_stage recipient
    @statistics = User.top_scores_report
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Tổng hợp kết quả sau lượt trận thứ hai vòng đấu bảng")
  end

  def staffs_top_scores_report_after_third_round_group_stage recipient
    @statistics = User.top_scores_report
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Bảng xếp hạng tổng thu nhập sau vòng bảng")
  end

  def staffs_top_scores_report_after_round_of_16 recipient
    @statistics = User.top_scores_report
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Bảng xếp hạng tổng thu nhập sau vòng 16 đội")
  end

  def staffs_top_scores_report_after_quarter_final recipient
    @statistics = User.top_scores_report
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Bảng xếp hạng tổng thu nhập sau vòng tứ kết")
  end

  def staffs_top_scores_report_after_semi_final recipient
    @statistics = User.top_scores_report
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Bảng xếp hạng tổng thu nhập sau vòng bán kết")
  end

  def staffs_top_scores_report_after_third_fourth recipient
    @statistics = User.top_scores_report
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Bảng xếp hạng tổng thu nhập sau trận tranh 3-4")
  end

  def staffs_top_scores_report_after_final recipient
    @statistics = User.top_scores_report
    mail(to: recipient, subject: "[NUS Event - World Cup 2022] Bảng xếp hạng tổng thu nhập mùa World Cup 2022")
  end
